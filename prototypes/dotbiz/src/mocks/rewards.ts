...
export const POLICY_CHANGELOG: PolicyChange[] = [
  ...
  {
    changedAt: "2026-05-08",
    changedBy: "CEO Settlement Direction",
    category: "Settlement",
    field: "Invoice Dispute 기능 폐기 — 송금 보류 빌미 차단",
    before: "Accounting이 Invoice 검증 후 분쟁 제기 → ELLIS 티켓 자동 생성 → 검토 워크플로우",
    after: "Invoice Dispute 기능 완전 삭제. 고객사가 인보이스에 대한 분쟁 제기 불가. SettlementPage의 Disputes 탭, '분쟁 제기' 버튼, '티켓 생성' 자동 라우팅, RaiseDisputeDialog 모두 비활성화. 예약 자체 이의(취소 요청 등)는 BookingsPage 또는 일반 티켓 트랙으로 처리.",
    reason: "Invoice 분쟁 제기 가능성이 고객사 송금 보류의 결정적 빌미가 됨. 분쟁 제기 워크플로우 자체를 차단하여 회수율 보호. 예약 단계 이의는 별도 트랙으로 분리.",
  },
  {
    changedAt: "2026-05-08",
    changedBy: "CEO Settlement Direction",
    category: "Settlement",
    field: "고객 송금 영수증 업로드 워크플로우 폐기 — 내부 매칭만",
    before: "Accounting이 송금 후 영수증(PDF/JPG) 업로드 → ELLIS가 입금 매칭 시 참고",
    after: "고객사 영수증 업로드 X. 송금 후 DOTBIZ 내부에서 금액 매칭만 운영. SettlementPage의 Payment Receipts 탭, UploadReceiptDialog 모두 비활성화.",
    reason: "고객사가 영수증 업로드 작업 안 함이 현실. 운영 표준 변경: 송금 → DOTBIZ가 은행 거래 내역과 인보이스 자동/수동 매칭 → 입금 확정. 고객 부담 작업 제거.",
  },
  {
    changedAt: "2026-05-08",
